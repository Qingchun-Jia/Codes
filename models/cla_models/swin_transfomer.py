...
class WindowAttention(nn.Module):
    ...
    def forward(self, x, mask: Optional[torch.Tensor] = None):
        ...
        """
        # [batch_size*num_windows, num_tokens, total_embed_dim]
        B_, N, C = x.shape
        ...
        relative_position_bias = self.relative_position_bias_table[self.relative_position_index.view(-1)].view(
            self.window_size[0] * self.window_size[1], self.window_size[0] * self.window_size[1], -1)  # [Mh*Mw,Mh*Mw,nH]
...
class SwinTransformer(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=4, in_chans=3, num_classes=1000,
                 ...
                 use_checkpoint=False, mode_select=0, use_fat_index=False, **kwargs ):
        super().__init__()
        self.mode_select = mode_select
        self.use_fat_index = use_fat_index
        ...
        if (mode_select in range(0, 5) or mode_select == 9) and not use_fat_index:
        ...
        elif mode_select in range(0, 5) and use_fat_index:
            self.fc1 = nn.Linear(self.num_features, self.num_features//2)
            self.head = nn.Linear(self.num_features//2+29, num_classes) if num_classes > 0 else nn.Identity()
        elif self.mode_select in range(5, 9) and use_fat_index:
            self.fc1 = nn.Linear(2*self.num_features, self.num_features//2)
            self.head = nn.Linear(self.num_features//2+29, num_classes) if num_classes > 0 else nn.Identity()
        else:
            self.fc1 = nn.Linear(2 * self.num_features, self.num_features//2)
            self.head = nn.Linear(self.num_features//2, num_classes) if num_classes > 0 else nn.Identity()
    ...
    def forward(self, x, fat_imgs=0, fat_index=0):
        x = self.forward_features(x)
        if self.mode_select in range(5, 9):
            fat_imgs = self.forward_features(fat_imgs)
            x = torch.cat((x, fat_imgs), dim=1)
        x = self.fc1(x)
        # x = self.dropout(x)
        if self.use_fat_index:
            # fat_index = self.fc2(fat_index)
            # fat_index = self.bn(fat_index)
            x = torch.cat((x, fat_index), dim=1)
            # x = self.bn(x)
        x = self.head(x)
        return x
...
def swin_small_patch4_window7_224(num_classes: int = 1000, mode_select=0, use_fat_index=False, **kwargs):
    # trained ImageNet-1K
