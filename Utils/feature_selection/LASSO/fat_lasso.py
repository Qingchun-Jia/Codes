...
for fold in range(1, 11):
    ...
    train_data = pd.read_csv(f'../../save_features/fat_fold_data/fold_{fold}/train_data_{fold}.csv')
    val_data = pd.read_csv(f'../../save_features/fat_fold_data/fold_{fold}/val_data_{fold}.csv')
    test0_data = pd.read_csv(f"../../save_features/fat_fold_data/fold_{fold}/test0_data_{fold}.csv")
    test1_data = pd.read_csv(f"../../save_features/fat_fold_data/fold_{fold}/test1_data_{fold}.csv")
    test2_data = pd.read_csv(f"../../save_features/fat_fold_data/fold_{fold}/test2_data_{fold}.csv")
    ...
    for alpha in tqdm(alphas):
        ...
        if auc > best_auc:
            best_auc = auc
            best_alpha = alpha
            joblib.dump(lasso, './pkl/lasso_fat_model.pkl')

    # 读取最好的LASSO模型
    lasso_model = joblib.load('./pkl/lasso_fat_model.pkl')
    ...
    print(f"train val test0，test1，test2auc结果分别为{train_lasso_auc}, {val_lasso_auc}, {test0_lasso_auc}, {test1_lasso_auc}, {test2_lasso_auc}")
...
for fold in range(1, 11):
    for i in range(len(dataset_name_list)):
        data = pd.read_csv(f'../../save_features/fat_fold_data/fold_{fold}/{dataset_name_list[i]}_data_{fold}.csv')
        ...
        df.to_csv(f'../selected_fat_fold_data/fold_{fold}/{all_csv_name[i]}_{fold}.csv', index=False)
