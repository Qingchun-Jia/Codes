...
for fold in range(1, 11):
    ...
    train_data = pd.read_csv(f'./nomogram_signature/fold_{fold}/train_data_{fold}.csv')
    val_data = pd.read_csv(f'./nomogram_signature/fold_{fold}/val_data_{fold}.csv')
    test0_data = pd.read_csv(f"./nomogram_signature/fold_{fold}/test0_data_{fold}.csv")
    test1_data = pd.read_csv(f"./nomogram_signature/fold_{fold}/test1_data_{fold}.csv")
    test2_data = pd.read_csv(f"./nomogram_signature/fold_{fold}/test2_data_{fold}.csv")

    train_img_ids = np.array(train_data.iloc[:, 0]).astype(int)
    train_labels = np.array(train_data.iloc[:, 1]).astype(int)
    train_features = np.array(train_data.iloc[:, 2:6]).astype(np.float)
    Logist_regressor = LogisticRegression()
    Logist_regressor.fit(train_features, train_labels)
    joblib.dump(Logist_regressor, f'./pkl/Logist_regressor_{fold}.pkl')
    print(fold)
    print(Logist_regressor.coef_)
    y_pred_train = Logist_regressor.predict_proba(train_features)[:, 1]
    train_data['Nomogram_pred'] = y_pred_train
    train_data.to_csv(f'./nomogram_signature/fold_{fold}/train_data_{fold}.csv', index=False)

    val_img_ids = np.array(val_data.iloc[:, 0]).astype(int)
    val_labels = np.array(val_data.iloc[:, 1]).astype(int)
    val_features = np.array(val_data.iloc[:, 2:6]).astype(np.float)
    y_pred_valid = Logist_regressor.predict_proba(val_features)[:, 1]
    val_data['Nomogram_pred'] = y_pred_valid
    val_data.to_csv(f'./nomogram_signature/fold_{fold}/val_data_{fold}.csv', index=False)

    test0_img_ids = np.array(test0_data.iloc[:, 0]).astype(int)
    test0_labels = np.array(test0_data.iloc[:, 1]).astype(int)
    test0_features = np.array(test0_data.iloc[:, 2:6]).astype(np.float)
    y_pred_test0 = Logist_regressor.predict_proba(test0_features)[:, 1]
    test0_data['Nomogram_pred'] = y_pred_test0
    test0_data.to_csv(f"./nomogram_signature/fold_{fold}/test0_data_{fold}.csv", index=False)

    test1_img_ids = np.array(test1_data.iloc[:, 0]).astype(int)
    test1_labels = np.array(test1_data.iloc[:, 1]).astype(int)
    test1_features = np.array(test1_data.iloc[:, 2:6]).astype(np.float)
    y_pred_test1 = Logist_regressor.predict_proba(test1_features)[:, 1]
    test1_data['Nomogram_pred'] = y_pred_test1
    test1_data.to_csv(f"./nomogram_signature/fold_{fold}/test1_data_{fold}.csv", index=False)

    test2_img_ids = np.array(test2_data.iloc[:, 0]).astype(int)
    test2_labels = np.array(test2_data.iloc[:, 1]).astype(int)
    test2_features = np.array(test2_data.iloc[:, 2:6]).astype(np.float)
    y_pred_test2 = Logist_regressor.predict_proba(test2_features)[:, 1]
    test2_data['Nomogram_pred'] = y_pred_test2
    test2_data.to_csv(f"./nomogram_signature/fold_{fold}/test2_data_{fold}.csv", index=False)
    ...
    train_fold_acc, train_fold_sens, train_fold_spec, train_fold_ppv, train_fold_npv = cal_acc_sens_spec_ppv_npv(0.8, train_labels,
                                                                                   y_pred_train)
    ...
    val_fold_acc, val_fold_sens, val_fold_spec, val_fold_ppv, val_fold_npv = cal_acc_sens_spec_ppv_npv(0.8, val_labels,
                                                                                   y_pred_valid)
    ...
    test0_fold_acc, test0_fold_sens, test0_fold_spec, test0_fold_ppv, test0_fold_npv = cal_acc_sens_spec_ppv_npv(0.8, test0_labels,
                                                                                   y_pred_test0)
    ...
    test1_fold_acc, test1_fold_sens, test1_fold_spec, test1_fold_ppv, test1_fold_npv = cal_acc_sens_spec_ppv_npv(0.8, test1_labels,
                                                                                   y_pred_test1)
    ...
    test2_fold_acc, test2_fold_sens, test2_fold_spec, test2_fold_ppv, test2_fold_npv = cal_acc_sens_spec_ppv_npv(0.8, test2_labels,
                                                                                   y_pred_test2)
